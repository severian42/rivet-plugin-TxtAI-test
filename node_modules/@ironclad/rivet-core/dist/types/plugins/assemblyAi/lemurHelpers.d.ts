import { Inputs, InternalProcessContext, PortId } from "../../index.js";
export declare function getApiKey(context: InternalProcessContext): string;
export declare function getLemurParams(inputs: Inputs, editorData: LemurNodeData): LemurParams;
export declare const lemurTranscriptIdsInputDefinition: {
    readonly id: PortId;
    readonly dataType: readonly ["string", "string[]", "any", "any[]"];
    readonly title: "Transcript IDs";
};
export declare const lemurEditorDefinitions: readonly [{
    readonly type: "dropdown";
    readonly label: "Final Model";
    readonly dataKey: "final_model";
    readonly options: readonly [{
        readonly value: "default";
        readonly label: "Default";
    }, {
        readonly value: "basic";
        readonly label: "Basic";
    }];
}, {
    readonly type: "number";
    readonly label: "Maximum Output Size";
    readonly dataKey: "max_output_size";
}];
export type FinalModel = 'default' | 'basic';
export type LemurParams = {
    transcript_ids: string[];
    context?: string;
    final_model?: FinalModel;
    max_output_size?: number;
};
export type LemurNodeData = {
    context?: string;
    final_model?: FinalModel;
    max_output_size?: number;
};
